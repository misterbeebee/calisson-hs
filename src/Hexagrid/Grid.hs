...
mkSpec radius shuffles entropy =
    ...
    trace ("pwo keys " ++ show positionsWithOrientation) $
    trace ("orientation keys int" ++ (show $ M.keys (cellOrientations))) $
    trace ("orientation keys pos " ++ (show $ map intToPos (M.keys (cellOrientations)))) $
